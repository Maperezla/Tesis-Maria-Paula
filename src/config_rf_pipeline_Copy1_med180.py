...
class RFPipelineConfig:
    ...
    normalization_excel: str
    normalized_raster_dir: str
    overwrite_norm_columns: bool
    nodata_output: float

    # Variables espaciales de contexto local: mediana en buffer
    enable_buffer_features: bool
    buffer_radius_m: float
    buffer_statistic: str
...
def load_config(path: Path) -> RFPipelineConfig:
    ...
    return RFPipelineConfig(
        ...
        # Normalización Min-Max
        normalization_method=str(
            cfg.get("normalization_method", "minmax_train_fold_final_model")
        ),
        normalization_excel=str(
            cfg.get("normalization_excel", "parametros_minmax_modelos.xlsx")
        ),
        normalized_raster_dir=str(
            cfg.get("normalized_raster_dir", "rasters_normalizados_minmax")
        ),
        overwrite_norm_columns=bool(
            cfg.get("overwrite_norm_columns", True)
        ),
        nodata_output=float(
            cfg.get("nodata_output", -9999.0)
        ),

        # Variables espaciales de contexto local: mediana en buffer
        enable_buffer_features=bool(
            cfg.get("enable_buffer_features", True)
        ),
        buffer_radius_m=float(
            cfg.get("buffer_radius_m", 180.0)
        ),
        buffer_statistic=str(
            cfg.get("buffer_statistic", "median")
        ),
        ...
        l8_fallback_next_available=bool(
            cfg.get("l8_fallback_next_available", True)
        ),
        ...
        s1_select_mode=str(
            cfg.get("s1_select_mode", "next_available_after_fire")
        ),

        # Modelo C
        model_c_enabled=bool(cfg.get("model_c_enabled", True)),
        s1_difference_same_month=bool(
            cfg.get("s1_difference_same_month", True)
        ),
        s1_difference_selection_strategy=str(
        ...
        out_gpkg_c=str(
            cfg.get("out_gpkg_c", "puntos_features_model_C_260507.gpkg")
        ),
        out_layer_c=str(
            cfg.get("out_layer_c", "puntos_features_model_C_260507")
        ),
        out_csv_c=str(
            cfg.get("out_csv_c", "dataset_C_complete_case_260507.csv")
        ),
        out_model_c=str(
            cfg.get("out_model_c", "modelo_RF_C_260507.pkl")
        ),
        out_report_c=str(
            cfg.get("out_report_c", "reporte_cv_C_260507.txt")
        ),
        ...
        seed_dataset_c=int(
            cfg.get("seed_dataset_c", cfg["seed_dataset_a"])
        ),
